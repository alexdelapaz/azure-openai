...
function footnoteReference(h, node2) {
  const footnoteOrder = h.footnoteOrder;
  const identifier = String(node2.identifier);
  const index2 = footnoteOrder.indexOf(identifier);
  const marker = String(
    index2 === -1 ? footnoteOrder.push(identifier) : index2 + 1
  );
  return h(
    node2,
    "a",
    ...
    [h(node2.position, "sup", [u("text", marker)])]
  );
}
...
function imageReference(h, node2) {
  ...
  const props = { src: (0, import_encode.default)(def.url || ""), alt: node2.alt };
...
function image(h, node2) {
  const props = { src: (0, import_encode2.default)(node2.url), alt: node2.alt };
...
function link(h, node2) {
  const props = { href: (0, import_encode4.default)(node2.url) };
  if (node2.title !== null && node2.title !== void 0) {
    props.title = node2.title;
  }
  return h(node2, "a", props, all2(h, node2));
...
function table(h, node2) {
  ...
  while (++index2 < rows.length) {
    ...
    let pos = node2.align ? align.length : row.length;
    const out = [];
    while (pos--) {
      const cell = row[pos];
...
function text3(h, node2) {
  return h.augment(
    node2,
    u("text", String(node2.value).replace(/[ \t]*(\r?\n|\r)[ \t]*/g, "$1"))
  );
}
...
function ReactMarkdown(options) {
  ...
  const processor = unified().use(remarkParse).use(options.remarkPlugins || options.plugins || []).use(remark_rehype_default, { allowDangerousHtml: true }).use(options.rehypePlugins || []).use(rehypeFilter, options);
